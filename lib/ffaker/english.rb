module Faker
  module English
    ...
    def sentence
      SENTENCES[rand(SENTENCES.count)] + '.'
    end

    def sentences(sentence_count = 3)
      (1..sentence_count).map { sentence }.join(' ')
    end

    SENTENCES = k "But I must explain to you how all this mistaken idea of denouncing
...
pleasures, or else he endures pains to avoid worse pains. But I must explain to
...
to avoid worse pains. But I must explain to you how all this mistaken idea of
...
our power of choice is untrammelled and when nothing prevents our being able to
do what we like best, every pleasure is to be welcomed and every pain avoided.
But in certain circumstances and owing to the claims.".gsub("\n", '').split('.').collect(&:strip)
  end
end
